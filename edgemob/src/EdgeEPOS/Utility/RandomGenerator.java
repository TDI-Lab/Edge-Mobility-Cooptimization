...
//        return s / m;
    ...
    public static double genBetaRandom(double alpha, double beta) {
    	BetaDistribution betaDis = new BetaDistribution(2.0, 5.0);//(alpha, beta)
        return betaDis.sample();
    }
    /**
     * 
     *
    ...
    public static void fillRandomBetaInArray(double[] input) {
	    double[] weight = new double[input.length];
	    double sum = 0;
	    for (int a = 0; a < input.length; a++) {
	        weight[a] = RandomGenerator.genBetaRandom(2.0, 5.0);
	        System.out.println("w"+weight[a]);
	        sum += weight[a];
	    }
	    for (int a = 0; a < input.length; a++) {
    ...
    public static void fillRandomBetaInArrayInRange(double[] input, int len) {
        double[] weight = new double[len];
        double sum = 0;

        // Scale and shift parameters
        double scale = 0.5; // Scale factor
        double shift = 0.5; // Shift factor to ensure values are in [0.5, 1]

        for (int a = 0; a < len; a++) {
            // Generate random value from beta distribution with parameters (2.0, 5.0)
            double rawValue = RandomGenerator.genBetaRandom(2.0, 5.0);

            // Scale and shift the raw value to fit in [0.5, 1]
            input[a] = scale * rawValue + shift;
            //System.out.println("w" + weight[a]); // Print the generated value
            //sum += weight[a]; // Calculate the sum of generated values
        }
