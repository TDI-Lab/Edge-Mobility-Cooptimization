...
import fog.entities.MobileDevice;
	...
	public class Run5 {
	    ...
	    public static void main(String[] args) throws FileNotFoundException {
	    	...
	    	String basePath = System.getProperty("user.dir") + File.separator + "src" + File.separator + "EdgeEPOS" + File.separator + "Setting" + File.separator + "input" + File.separator;
	    	String cityconfigfile = basePath + "CityConfig.json";
	    	System.out.println("conf path:"+cityconfigfile);
			...
			int [] nodes = new int[numOnNodes];
			
			for (int i = 0; i<12; i++) {
				int arindex = 0;
				Set<Integer> sample = generatesample(numOnNodes);
				
				for (int value : sample) {
				    nodes[arindex++] = value;
				}
				avgruns.add(new AvgRuns(nodes, capRatio[0]/numOnNodes));//([],0.8/30)
			}
			
			 Utility.print2File(avgruns);
			    
			Constraint cons = new Constraint(Constants.numNodes, Constants.onoff, Constants.target, Constants.capacityfactor, numOnNodes);
			...
			Constants.setPaths(numOnNodes+"-"+capRatio[0]+"-"+k);
	        ...
	        for (int profile = 0 ; profile < Constants.PROFILE_NUM ; profile++) {//interval for IoT PROFILE update 
	              ...
	              for (int j = 0; j<Constants.EposRunPerWorkload; j++) {//vehicles' update (epos run) interval
						...
						if(!cons.setAgentsConstraintRandomly(avgruns.get(k).allNodesCapRatio, avgruns.get(k).onNodes, capRatio[0]/numOnNodes)) {
							notEnoughRsc = true;
							break;
						}
	    ...
	    private static Set<Integer> generatesample(int numberOfValues) {
			Set<Integer> uniqueValues = new HashSet<>();

   	        while (uniqueValues.size() < numberOfValues) {
    	            int randomValue = RandomGenerator.genUniformRandomBetween(0,112);
		...
		public static void setServerCloudlets(List<FogDevice> serverCloudlets) {
			Run5.serverCloudlets = serverCloudlets;
		}
		...
		public static void setCoordDevices(Coordinate coordDevices) {
			Run5.coordDevices = coordDevices;
		}
		...
		public static void setMaxSmartThings(int maxSmartThings) {
			Run5.maxSmartThings = maxSmartThings;
		}
