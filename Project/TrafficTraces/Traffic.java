...
//        for (int a = 0; a < Constants.numServices; a++) {
...
//        method.traffic.arrivalInstructionsFog[a][j] = Constants.L_P[a] * method.traffic.lambda_in[a][j] * method.x[a][j];
//    }
//
//    /**
...
//        for (int k = 0; k < Constants.numCloudServers; k++) {
...
//        method.traffic.arrivalInstructionsCloud[a][k] = Constants.L_P[a] * method.traffic.lambdap_in[a][k] * method.xp[a][k];
//    }
//
//    /**
...
//        double tempSum = 0;
//        for (Integer j : Constants.H_inverse[a][k].elemets) {
...
//    }
    ...
    private static void distributeTraffic(double trafficPerNodePerService, double[][] targetTraffic) {
        double totalTraffic = trafficPerNodePerService * Constants.numFogNodes * Constants.numServices;
        double trafficForCurrentService;
        
        System.out.println(" traffic: "+trafficPerNodePerService+" totalTraffic "+ totalTraffic);
        
        double[] nodeTrafficPercentage = new double[Constants.numFogNodes];
        for (int a = 0; a < Constants.numServices; a++) {
            trafficForCurrentService = totalTraffic * Constants.ServiceTrafficPercentage[a];
            ArrayFiller.fillRandomPDFInArray(nodeTrafficPercentage);
            for (int j = 0; j < Constants.numFogNodes; j++) {
                targetTraffic[a][j] = trafficForCurrentService * nodeTrafficPercentage[j];
            // System.out.println(" aj "+ targetTraffic[a][j]+" rate: "+Constants.ServiceTrafficPercentage[a]);
    ...
    private static void distributeTrafficSmartThings(double trafficPerNodePerService, double[] targetTraffic) {
        ...
        //for (int a = 0; a < Constants.numServices; a++) {
            ...
            for (int j = 0; j < Constants.numSmartThings; j++) {
                targetTraffic[j] = totalTraffic * nodeTrafficPercentage[j];
                System.out.println(" aj "+j+" tt "+ targetTraffic[j]);
        
       //}
        }
            System.out.println(" fini_____");
    }
    
    public static void distributeTraffic1(double trafficPerNodePerService) {
    	distributeTrafficSmartThings(trafficPerNodePerService, Constants.globalTraffic1);
    }
    /**
     * Distributes the given traffic that is for one service on one node
    ...
    public static void distributeTraffic(double trafficPerNodePerService) {
        distributeTraffic(trafficPerNodePerService, Constants.globalTraffic);
    }
    ...
    private static void distributeTraffic(Double[] combinedTrafficPerNode, double[][] targetTraffic) {
        for (int j = 0; j < Constants.numFogNodes; j++) {
        	...
        	for (int a = 0; a < Constants.numServices; a++) {
                targetTraffic[a][j] = combinedTrafficPerNode[j] * Constants.ServiceTrafficPercentage[a];
                 }
        }
    }
    ...
    public static void distributeTraffic(Double[] combinedTrafficPerNode) {
        distributeTraffic(combinedTrafficPerNode, Constants.globalTraffic);
    }
    ...
    public static void setTrafficToGlobalTraffic(Method method) {
        for (int a = 0; a < Constants.numServices; a++) {
            for (int j = 0; j < Constants.numFogNodes; j++) {
                method.traffic.lambda_in[a][j] = Constants.globalTraffic[a][j];
            }
        }
        enlargeTrafficFog(method);
    }
